...
class PostDetail(DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            send_mail(subject, message, 'Skillfactory MessageBoard <96bat69@rambler.ru>', recipient_list)
    ...
    def send_code_view(request):
        ...
        if email:
            ...
            return JsonResponse({'status': 'success', 'code': code})  # Не отправляйте код в ответ, это небезопасно
        return JsonResponse({'status': 'error', 'message': 'Email не предоставлен.'})
